...
export const zsTypeExprMatcher = ztSchemaWorld
    .matcher(ScopedZtMatcherState.empty)
    .cases({
        ...
        [AnyTypeKind.ZodTuple](node) {
            ...
            if (node._def.rest) {
                members.push(
                    tf.createRestTypeNode(this.recurse(node._def.rest))
                )
            }
