...
import { ZsTypeExpressionTable, zsSchemaDomain } from "../zt-types"
import { tf } from "../tf"
...
export const zsTypeExprMatcher = zsSchemaDomain.matcher(TypeExprContext).cases({
    [ZsTypeKind.ZsEnum](node) {
        return this.get(node)
    },
    [ZsTypeKind.ZsImportedType](node) {
        return this.get(node)
    },
    ZodNativeEnum(node) {
    ...
    [AnyTypeKind.ZsInstantiation](node) {
        ...
        const genericType = this.get(node._def.instance) as TypeReferenceNode
